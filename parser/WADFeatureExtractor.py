...
class WADFeatureExtractor(object):
    ...
    def _rescale_coord(self, x_du, y_du, wad_features, size = [256, 256], pad = True):
        ...
        if pad:
            ...
            pad_x = np.ceil((15000 - x_max)/150) if x_max/15000 < 1 else np.floor((size[0]-1-x_max)/2) if x_max<55 else 0
            pad_y = np.ceil((15000 - y_max)/150) if y_max/15000 < 1 else np.floor((size[1]-1-y_max)/2) if y_max<55 else 0
    ...
    def compute_maps(self, level_dict, wad_features):
        ...
        level_dict['text'] = self.draw_textmap(maps)
        return maps
